const matrixData = JSON.parse(localStorage.getItem('matrixData') || '[]') as number[][];
...
const divideMatrixByScalar = (matrix: number[][], scalar: number) => {
...
const adjointMatrix = (matrix: number[][]) => {
	const size = matrix.length;
	const adjoint: number[][] = Array.from({ length: size }, () => Array(size).fill(0));
...
const inverseMatrix = (matrix: number[][]) => {
...
try {
	...
	resultDisplay.innerHTML = `<p>Error: ${(error as Error).message}</p>`;
}